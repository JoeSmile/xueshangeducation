            ...
            ProcessItem.map(item => 
              <WrapItem key={item.reason}>
                <Box
                  ...
                  bg={`url(${item.bgImage}) no-repeat center center`}
                >
                  <Image src={item.icon} objectFit='cover' alt='icon'/>
                  <Text fontSize='16px' fontWeight='600' marginY='0'>{item.reason}</Text>
                  <Divider variant='dashed' marginY='10px' opacity='1' borderColor={item.dividerColor} />
